class StrictLucBatProcessor(LogitsProcessor):
    def __init__(self, tokenizer, max_total_lines=2):
        ...
        self.space_token_id = tokenizer.encode(' ', add_special_tokens=False)[0] # Token dấu cách
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # Cờ báo hiệu từ cuối cùng đã hoàn chỉnh chưa (có khoảng trắng ở cuối không)
        is_word_completed = current_line.endswith(' ') if current_line else True
        ...
        for token_id in top_indices:
            ...
            if '\n' in token_str or token_id == self.eos_token_id:
                if current_word_count < target_length: is_valid = False 
                    ...
                    # Cấm xuống dòng nếu từ cuối cùng chưa hoàn chỉnh (hoặc chưa có nguyên âm)
                    if current_words and (not any(c in self.vowels for c in current_words[-1].lower()) or not is_word_completed):
                        # CHỈ CHO PHÉP XUỐNG DÒNG NẾU TỪ CUỐI CÙNG ĐÃ HOÀN TẤT VÀ CÓ DẤU CÁCH HOẶC TỰ MÔ HÌNH SINH RA \N
                        pass # Sẽ điều chỉnh điểm bên dưới
        ...
        # CHỈ KHUYẾN KHÍCH XUỐNG DÒNG NẾU TỪ ĐÃ HOÀN CHỈNH
        if current_word_count == target_length:
            if current_words and any(c in self.vowels for c in current_words[-1].lower()):
                # Nếu từ đã có nguyên âm, ta cho phép xuống dòng/EOS, NHƯNG...
                # Chỉ ép mạnh khi dấu cách đã xuất hiện (nghĩa là AI đã chốt xong từ)
                if is_word_completed or token_str.startswith(' '):
                     if is_finishing_poem: scores[0, self.eos_token_id] += 50.0 
                     else: scores[0, self.nl_token_id] += 25.0
                else:
                     # Nếu đang gõ dở từ, phạt nhẹ điểm xuống dòng để AI gõ cho xong
